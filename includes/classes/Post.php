	...
	public function submitPost($body, $user_to, $imageName) {
		...
		if($check_empty != "") {
			...
			$stopWords = "a about above across after again against all almost alone along already
			 also although asa always among am an and another any anybody anyone anything anywhere are 
			 ...
			 both but by c college school him came can cannot case cases certain certainly clear clearly come could
			 d did differ different differently do does done down down downed downing downs during
			 e each early either held hold hai inform end ended ending ends enough even evenly ever every everybody
			 ...
			 grouping groups h had has have having hello is are  he her here herself high high high higher
             ...
             hate sleepy love on reason for some little yes bye choose";
...
public function loadPostsTrends($trendingWord, $limit) {
		...
		$data_query = mysqli_query($this->con, "SELECT * FROM posts WHERE deleted='no' AND body LIKE '%$trendingWord%' ORDER BY id DESC");
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				if($user_logged_obj->isFriend($added_by)){
					...
					if($imagePath != "") {
						$imageDiv = "<embed src='$imagePath' class='postedImage'>
									</embed>";
					}
					else {
						$imageDiv = "";
